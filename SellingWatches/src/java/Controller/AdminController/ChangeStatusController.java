...
public class ChangeStatusController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean updateResult = dbContext.updateUserStatus(userID, status);

        if (updateResult) {
            // Handle successful update, e.g., redirecting to the user list page
            // Lấy URL của trang trước đó
            String referer = request.getHeader("Referer");

            // Chuyển hướng người dùng trở lại trang trước
            response.sendRedirect(referer); // Đảm bảo referer không null và hợp lệ trước khi sử dụng // Adjust the redirection URL as needed
        } else {
            // Handle failed update, optional
